...
void *SortLocalList(void *arg){
    ...
    qsort(&list_t[0], my_list_size_t, sizeof(int), compare_int);
...
void sort_list(int q) {
    ...
    // Sort local lists
    LocalSortData *LSD= (LocalSortData *) malloc (num_threads* sizeof(LocalSortData));
    ...
    // Sort list
    MergeSortData *MSD = (MergeSortData *) malloc(num_threads* sizeof(MergeSortData));
