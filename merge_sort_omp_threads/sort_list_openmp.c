...
void sort_list(int q) {
    ...
    // Sort local lists
    #pragma omp parallel for private(my_id, my_list_size) shared(ptr, list)
    for (my_id = 0; my_id < num_threads; my_id++) {
...
if (DEBUG) print_list(list, list_size); 
    ...
    for (level = 0; level < q; level++) {
        ...
        #pragma omp parallel private(my_id, my_blk_size, my_own_blk, my_own_idx, my_search_blk, my_search_idx, my_search_idx_max, my_write_blk,my_write_idx, idx,my_search_count, i_write, i) shared(work, list, level, np, ptr)
        {
            #pragma omp for 
            for (my_id = 0; my_id < num_threads; my_id++) {
