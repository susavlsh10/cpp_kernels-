...
#define MAX_LIST_SIZE_PER_PROC	1474560000
...
// Search for largest element in a sorted list which is larger than pivot
...
void HyperCube_Class::check_list() {
    int tag = 0;
    int max_nbr = INT_MAX;		// Assumes list contains non-negative integers; 6-21-2017
	...
	if (error == 0) {
	    ...
	    printf("[Proc: %0d] Error encountered. The list has not been sorted correctly in descending order.\n", my_id);
	}
    }
}
//
...
void HyperCube_Class::HyperCube_QuickSort() {
	...
	// Search for largest element in list which is smaller than pivot
	// Upon return:
	//   list[0 ... idx-1] >= pivot
    ...
    // MPI-2: Send number of elements greater than pivot
	    ...
	    // ***** Add MPI call here *****
		MPI_Send(&list_size_gt, 1, MPI_INT, nbr_k, 1, MPI_COMM_WORLD);

	    // MPI-3: Receive number of elements less than or equal to pivot

	    // ***** Add MPI call here *****
		MPI_Recv(&nbr_list_size, 1, MPI_INT, nbr_k, 1, MPI_COMM_WORLD, &status);
	    ...
	    // ***** Add MPI call here *****
		MPI_Send(&list[idx], list_size_gt, MPI_INT, nbr_k, 1, MPI_COMM_WORLD);

	    // MPI-5: Receive neighbor's list of elements that are less than or equal to pivot

	    // ***** Add MPI call here *****
		MPI_Recv(nbr_list, nbr_list_size, MPI_INT, nbr_k, 1, MPI_COMM_WORLD, &status);
	    ...
	    // ***** Add MPI call here *****
		MPI_Send(&list_size_leq, 1, MPI_INT, nbr_k, 1, MPI_COMM_WORLD);
	    ...
	    // ***** Add MPI call here *****
		MPI_Send(list, list_size_leq, MPI_INT, nbr_k, 1, MPI_COMM_WORLD);
